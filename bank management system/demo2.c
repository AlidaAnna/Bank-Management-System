...
void delete(struct bank *b)
{
char dname[50],dpass[50];
char fname[50],fpass[50];
int flag=0;
fp=fopen("Bank.txt","r");
fd=fopen("delete.txt","w");
if(fp==NULL||fd==NULL)
{
printf("Error:");
return;
...
printf("\nEnter your password:");
scanf("%s",dpass);
while(fscanf(fp,"%s%s%d",fname,fpass,&fbal)==3)
{
if(strcmp(fname,dname)==0&&strcmp(fpass,dpass)==0)
{
flag=1;
}
else
{
fprintf(fd,"%s%s%d",fname,fpass,fbal);
}
}
fclose(fp);
fclose(fd);
